...
def trainloss(o_label, l_label):
    ...
    with tf.name_scope('losses'):
        total_loss = slim.losses.softmax_cross_entropy(o_label, l_label)
        tf.summary.scalar('total_loss', total_loss)
    return total_loss


def train(model, images, labels, images_validation, labels_validation, x_shape, y_shape, saved_path, num_per_valid,
    ...
    label_validation = tf.reshape(labels_validation, y_shape)
    ...
    accuracy_validation = slim.metrics.accuracy(tf.to_int32(tf.argmax(predictions_validation, 1)),
                                                tf.to_int32(tf.argmax(label_validation, 1)))
...
if __name__ == '__main__':
    ...
    train(Inception_ResNet, images, labels, images_validation, labels_validation,
